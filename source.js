...
var studentData = [{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
function populateTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        // Combine image with first name and last name
        var nameCell = row.insertCell();
        var imageElement = document.createElement("img");
        imageElement.src = data[i].img_src;
        imageElement.alt = data[i].name + "'s Image";
        nameCell.appendChild(imageElement);
        nameCell.appendChild(document.createTextNode(data[i].first_name)); // Display first name
        nameCell.appendChild(document.createTextNode(" " + data[i].last_name)); // Display last name
        
        row.insertCell().textContent = data[i].gender;
        row.insertCell().textContent = data[i].class;
        row.insertCell().textContent = data[i].marks;
        row.insertCell().textContent = data[i].passing?"Passing":"Failed";
...
document.querySelector('.search-button').addEventListener('click', function() {
    ...
    var filteredData = studentData.filter(function(student) {
        return student.first_name.toLowerCase().includes(searchTerm) ||student.last_name.toLowerCase().includes(searchTerm) || student.email.toLowerCase().includes(searchTerm);
    });
...
document.getElementById('sortByNameAZ').addEventListener('click', function() {
    var sortedData = studentData.slice().sort((a, b) => a.first_name.localeCompare(b.first_name));
    populateTable(sortedData);
});

document.getElementById('sortByNameZA').addEventListener('click', function() {
    var sortedData = studentData.slice().sort((a, b) => b.first_name.localeCompare(a.first_name));
    populateTable(sortedData);
});


document.getElementById('sortByMarks').addEventListener('click', function() {
    var sortedData = studentData.slice().sort((a, b) => a.marks - b.marks);
    populateTable(sortedData);
});

document.getElementById('sortByPassing').addEventListener('click', function() {
    var sortedData = studentData.slice().sort((a, b) => b.passing - a.passing);
    populateTable(sortedData);
});

document.getElementById('sortByClass').addEventListener('click', function() {
    var sortedData = studentData.slice().sort((a, b) => {
        // Extract numeric part of class name
        var classA = parseInt(a.class);
        var classB = parseInt(b.class);
...
document.getElementById('sortByGender').addEventListener('click', function() {
    var sortedData = studentData.slice().sort((a, b) => a.gender.localeCompare(b.gender));
    populateTable(sortedData);
});

// Initial population of table with all data
